...
goblin_l = pygame.image.load('images/npc_2_l.png')
goblin_l.set_colorkey((255, 255, 255))
goblin_r = pygame.image.load('images/npc_2_r.png')
...
pol = pygame.image.load('images/pol_2.png')
pol.set_colorkey((255, 255, 255))
wall_hor = pygame.image.load('images/wall_hor_2.png')
wall_hor.set_colorkey((255, 255, 255))
wall_ver = pygame.image.load('images/wall_ver_2.png')
...
npc_bullet = pygame.image.load('images/npc_bullet_2.png')
...
class Nepice:
    ...
    def attack(self):
        ...
        bullets.append(NPC_bullet(self.x, self.y, aim_x + randint(-200, 200), aim_y + randint(-200, 200)))
...
class Goblin(Nepice):
    ...
    def draw(self, hp):
        ...
        pygame.draw.rect(screen, (255, 0, 0), (self.x, self.y - 10, int(70 * hp / 20), 10))
...
class NPC_bullet:
    ...
    def draw(self):
        ...
        if self.x < 30 or self.x > 1112 or self.y < 30 or self.y > 680:
            ...
            return True
        if int(self.x + 25) in list(range(int(hero_2.x), int(hero_2.x + 70))) and int(self.y + 10) in list(range(int(hero_2.y), int(hero_2.y + 50))) and not GG_2:
            hero_2.get_damage(self.damage)
            pygame.draw.circle(screen, (255, 0, 0), (self.x + 30, self.y + 55), 15)
            return True
        elif int(self.x + 25) in list(range(int(hero.x), int(hero.x + 70))) and int(self.y + 10) in list(range(int(hero.y), int(hero.y + 50))) and not GG_1:
        ...
        g_l_r = npc_bullet.get_rect(
            topleft=(self.x, self.y))
        if self.start_x < self.end_x:
            rot_image = pygame.transform.rotate(npc_bullet, self.angle)
        else:
            rot_image = pygame.transform.rotate(npc_bullet, -self.angle)
...
while running:
    ...
    if len(npc) == 0 and waves_count < waves and room == 2:
        if freeze_waves >= 90:
            npc = []
            for i in range(choice(list(range(2 + waves_count, 7 + waves_count)))):
    ...
    if rs >= 0 and rs < 60 and room == 1:
        rs += 1
        n = pygame.font.Font(None, 120)
        t = n.render('LEVEL 1-2!', True, (0, 0, 255))
